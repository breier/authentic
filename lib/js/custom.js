...
function editProfile (postObject) {
	if (typeof postObject != 'object') var postObject = {'ajax': 1};
	$.ajax({
		url: ajaxFolder +'/session/profile.php',
		type: 'POST',
		data: postObject,
		success: function (response) {
			if (response[0]=='[') {
				...
				if ($("#profile").length) $("#profile").remove();
				$("body").append(response);
				if ($("#profile").length) $("#profile").modal("show");
			}
		}
	});
}
...
function numberAnimate (object, nStart, nEnd) {
	$({ counter: nStart }).animate({ counter: nEnd }, {
   	duration: 1000,
   	easing: 'swing',
   	step: function () { $(object).html(Math.ceil(this.counter)); }
	});
}
	...
	$.ajax({
		...
		success: function (response) {
			if (response[0]!='[') alertPNotify ('alert-danger', response, 5000);
				...
				if (rows_content[0]['total'] > rowsPerPage) {
					$("#pagination").prev("button").attr("disabled", (pageNumber == 1));
					$("#pagination").empty();
					var listPages = Math.ceil(rows_content[0]['total'] / rowsPerPage);
					for (var i=1; i <= listPages; i++) {
						...
						if ($("#pagination button.active").html() > 2) {
							...
							$("#pagination button.active").prev("button").addClass("btn btn-default").on("click", null, "input", list_pageNumber);
						} if ($("#pagination button.active").html() < (listPages - 1)) {
							$("#pagination button.active").after("<button>...</button>");
							$("#pagination button.active").next("button").addClass("btn btn-default").on("click", null, "input", list_pageNumber);
						}
...
function list_paginate (number) {
	...
	if (typeof number == 'number') {
		...
		else if (number == 0) window.location = searchArray[0] +'#'+ searchString +'#'+ (parseInt(pageNumber) + 1) +'#'+ sortOrder;
		else if (number > 0) window.location = searchArray[0] +'#'+ searchString +'#'+ number +'#'+ sortOrder;
...
/* ----- Define AJaX pagination function for List Users ----- */
function list_pageNumber (extendButton) {
	if (typeof extendButton != 'object') return false;
	if ($(".extend_page_btn").length) return $(".extend_page_btn").remove();
	var extendDiv = '<div class="extend_page_btn"><input class="form-control" type="number" placeholder="#" id="page_number" />';
	extendDiv += '<button class="btn btn-info">&gt;</button></div>';
	$(extendButton.target).after(extendDiv);
	$("#page_number").focus().on("keyup", function (event) {
		var keyCode = (event.keyCode) ? (event.keyCode) : (event.which);
   	if (keyCode == 13) {
			list_paginate(parseInt($("#page_number").val()));
			$(".extend_page_btn").remove();
   	}
...
/* ----- Define pagination function for Tools' Tables ----- */
function tools_paginate (number) {
	var tablePaginate = $(".x_content table.table");
	var activePage = tablePaginate.next("div").find(".active").html();
	var newPage = 0;
	if (typeof number == 'number') {
		if (number == -1) {
			if (activePage > 1) {
				tablePaginate.next("div").find(".active").removeClass("active").prev("button").addClass("active");
				var newPage = tablePaginate.next("div").find(".active").html();
			}
		} else {
			if (activePage != tablePaginate.next("div").find("#pagination button:last").html()) {
				tablePaginate.next("div").find(".active").removeClass("active").next("button").addClass("active");
				var newPage = tablePaginate.next("div").find(".active").html();
			}
		}
	} else if (typeof number == 'object') {
		var newPageObject = number.target;
		if (activePage != $(newPageObject).html()) {
			tablePaginate.next("div").find(".active").removeClass("active");
			$(newPageObject).addClass("active");
			var newPage = $(newPageObject).html();
		}
	} if (newPage == 0) return false;
	if (newPage == 1) {
		tablePaginate.next("div").find("button:first").attr('disabled', true);
	} else tablePaginate.next("div").find("button:first").attr('disabled', false);
	if (newPage == tablePaginate.next("div").find("#pagination button:last").html()) {
		tablePaginate.next("div").find("button:last").attr('disabled', true);
	} else tablePaginate.next("div").find("button:last").attr('disabled', false);
	tablePaginate.find("tbody tr").each(function(index, element) {
		if (parseInt((index/rowsPerPage)+1) == newPage) $(element).removeClass("hide");
		else $(element).addClass("hide");
	});
}

/* ----- Define Edit Plan Modal for Manage Plans ----- */
function tools_editPlan (planID, trObject) {
	var editForm = $("#modal_plan_edit .modal-body form");
	editForm.find("#plan_id").val(planID);
	editForm.find("#plan").val($(trObject).find("td:nth(0) span").html());
	editForm.find("#price").val($(trObject).find("td:nth(1) span").html());
	editForm.find("#media").val($(trObject).find("td:nth(2) span").attr("label"));
	editForm.find("#download").val($(trObject).find("td:nth(3) span").html());
	editForm.find("#upload").val($(trObject).find("td:nth(4) span").html());
	editForm.find("#assured").val($(trObject).find("td:nth(5) span").html());
	$("#modal_plan_edit").modal("show");
	editForm.find("#media").selectpicker("refresh");
	editForm.find("button.btn-danger").off("click");
	editForm.find("button.btn-danger").on("click", null, {'planID': planID, 'planName': editForm.find("#plan").val()}, tools_deletePlan);
}

/* ----- Define Confirm and Delete Modal for Manage Plans ----- */
function tools_deletePlan (event) {
	var planID = event.data.planID;
	var planName = event.data.planName;
...
function settings_saveSingle (formObject) {
	var formSelector = (typeof formObject == 'object') ? ($(formObject)) : ($('.x_content form'));
	formSelector.find("input[type=text]").each(function (index, element) {
		if ($(element).val() != $(element).prop("defaultValue")) $(element).attr("data-changed", true);
	});
	formSelector.find("select").each(function (index, element) {
		$(element).find("option").each(function (idx, elm) {
	...
	if (!formSelector.find("*[data-changed]").length) $("#modal_nochange").modal("show");
	else {
		var finalValidation = true;
		formSelector.find('input[data-changed]').each(function (index, element) {
		...
		formSelector.find("input:not([data-changed]):not([data-required]), select").attr('disabled', true);
		formSelector.find('input[data-changed]').each(function (index, element) {
			formSelector.find('input[name="'+ $(element).attr("name") +'"]').attr('disabled', false);
		});
		formSelector.find('select[data-changed]').each(function (index, element) { // Fix select label array (not multiple select)
			formSelector.append($('<input>', {'type': "hidden", 'name': $(element).attr("name"), 'value': $(element).attr("name").replace('[]', '')}));
			formSelector.append($('<input>', {'type': "hidden", 'name': $(element).attr("name"), 'value': $(element).val()}));
		});
		return true;
	} return false;
}

	...
	$.ajax({
		...
		success: function (response) {
			if (response[0]!='[') alertPNotify ('alert-danger', response, 5000);
				...
				} $("#users_table tbody tr:has(a.disabled) td").addClass('disabled');
...
function list_handleDuplicates (parentObject) {
	...
	var controlClasses = ($(parentObject).is(".modal-body")) ? ("col-md-6 col-sm-6 col-xs-12") : ("form-group");
	$(parentObject).find(".duplicated").each(function (index, element) {
		...
		$(element).prev("div").before($("<div>", {"class": controlClasses +" control"}));
		$(secondElementInput).attr("disabled", true);
		var controlElement = $(element).prev("div").prev("div");
		if (!$(parentObject).is(".modal-body")) {
			$(controlElement).append($("<label>", {"class": "control-label col-md-3 col-sm-3 col-ms-3 col-xs-12"}));
			$(controlElement).append($("<div>", {"class": "col-md-6 col-sm-6 col-ms-6 col-xs-12"}));
		} else $(controlElement).append($("<div>", {"class": "col-md-12 col-sm-12 col-xs-12 form-group"}));
...
function list_delete (userID) {
	if (typeof userID == 'undefined') return $("#modal_delete .modal-body form").attr("action", window.location.href).submit();
	...
	$("#modal_delete .modal-body form input[name='id']").val(userID);
...
function tools_ontSelect (ontInfo) {
	...
	$("#ont_port").val(ontInfoArray[0]);
	$("#ont_sn").val(ontInfoArray[1]);
}
...
function tools_ontCustomerSelect (ontCustomerInfo) {
	...
	$("#activate_pppoe").attr("disabled", false);
	$("#activate_bridge").attr("disabled", false);
...
function tools_ontActivate (activationType) {
	...
	var postData = {'ajax': 1, 'action': 'activate', 'type': activationType, 'ont_port': $("#ont_port").val(), 'ont_sn': $("#ont_sn").val(),
